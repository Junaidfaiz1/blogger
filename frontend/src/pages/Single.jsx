  ...
  return (
    <div className="container mx-auto p-8 flex flex-col lg:flex-row bg-gray-100 min-h-screen">
      {data.map((blog, index) => (
        <div
          key={index}
          className="lg:w-3/4 w-full bg-gray-200 rounded-xl p-6 shadow-xl flex flex-col"
        >
          ...
          <div className="flex items-center mt-4 bg-gray-200 p-4 rounded-xl shadow-inner">
            <img
              src={
                blog.authorProfilePicture || "https://via.placeholder.com/50"
              } // Use author's avatar or a placeholder
              alt="Author Profile"
      ...
      <Suspense fallback={"Loading..."}>
        <div className="lg:w-1/4 w-full mt-8 lg:mt-0 lg:ml-8 bg-gray-200 rounded-xl p-6 shadow-xl">
          <RelatedPosts category={data[0]?.category} />
        </div>
      </Suspense>
    </div>
  );
